  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      <div className="mb-12 text-center">
        <h1 className="text-4xl font-extrabold text-indigo-950 mb-4 tracking-tight">School News & Articles</h1>
      ...
      {newsItems.length === 0 ? (
        <div className="text-center py-32 bg-white rounded-3xl border border-dashed border-slate-300">
           <span className="text-6xl text-slate-200 block mb-6">🗞️</span>
           <p className="text-xl text-slate-500 font-medium">No news articles published yet.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {newsItems.map((news: any) => (
            <Link href={`/lesson/${news._id.toString()}`} key={news._id.toString()}>
              <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all border border-slate-100 flex flex-col md:flex-row gap-6 lg:gap-8 group cursor-pointer items-start">
                
                {/* News Thumbnail */}
                <div className="w-full md:w-1/3 h-48 md:h-full min-h-[12rem] bg-indigo-50 rounded-2xl relative overflow-hidden shrink-0">
                ...
                <div className="flex flex-col flex-grow py-2">
                  ...
                  <h2 className="text-2xl font-bold text-indigo-950 mb-3 group-hover:text-indigo-700 transition-colors">
                    {news.title}
                  </h2>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-50">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-slate-700">{news.creator?.name || 'Unknown Author'}</span>
                    </div>
